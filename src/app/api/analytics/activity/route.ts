...
import { getRecentActivity, getTopLeads } from '@/lib/services/analytics-service';
...
export async function GET() {
  try {
    ...
    if (!recentResult.success) {
      return NextResponse.json<ApiResponse<never>>({ error: recentResult.error }, { status: 500 });
    }

    if (!topResult.success) {
      return NextResponse.json<ApiResponse<never>>({ error: topResult.error }, { status: 500 });
    }
