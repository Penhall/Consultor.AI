...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!statusResult.success) {
      return NextResponse.json<ApiResponse<never>>({ error: statusResult.error }, { status: 500 });
    }
    ...
    if (!profileResult.success) {
      return NextResponse.json<ApiResponse<never>>({ error: profileResult.error }, { status: 500 });
    }
