...
async function getWabaIdFromToken(accessToken: string): Promise<{ data?: string; error?: string }> {
  ...
  if (!wabaScope?.target_ids?.[0]) {
    return {
      error: 'No WhatsApp Business Account found. Please ensure one was selected.',
    };
  }
...
async function getPhoneNumberDetails(
  ...
  if (!phoneData.data?.[0]) {
    return {
      error: 'No phone numbers found. Please add a phone number to your WABA.',
    };
  }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const consultantCheck = await verifyConsultant(supabase, consultant_id, user.id);
    if (consultantCheck.error) {
      return NextResponse.json({ error: consultantCheck.error }, { status: 404 });
    }
    ...
    const { id: phone_number_id, display_phone_number, verified_name } = phoneResult.data;
