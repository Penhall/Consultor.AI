...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!from || !text) {
      return NextResponse.json({ error: 'Parâmetros obrigatórios: from, text' }, { status: 400 });
    }

    // Use service role client: this endpoint is unauthenticated (no browser cookies),
    // so createClient() (anon key + RLS) would return zero rows for consultants.
    // createServiceClient() bypasses RLS, allowing the mock to find any consultant.
    ...
    if (!conversation) {
      ...
      if (!flow) {
        return NextResponse.json({ error: 'Nenhum flow ativo encontrado' }, { status: 404 });
      }
    ...
    if (!conversation.flow_id) {
      return NextResponse.json({ error: 'Conversa sem flow associado' }, { status: 400 });
    }
    ...
    if (!flow || !flow.definition) {
      return NextResponse.json({ error: 'Flow não encontrado ou inválido' }, { status: 404 });
    }
