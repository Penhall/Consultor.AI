...
export default function LeadDetailPage({ params }: { params: { id: string } }) {
  ...
  const loadConversations = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/leads/${params.id}/conversations`, { credentials: 'include' });
      if (res.ok) {
        const data = await res.json();
  ...
  const loadFollowUps = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/leads/${params.id}/follow-ups`, { credentials: 'include' });
      if (res.ok) {
        const data = await res.json();
  ...
  return (
    <div className="space-y-6 p-8">
      ...
      {/* Lead Details */}
      <LeadDetail lead={lead} onStatusChange={handleStatusChange} onUpdate={handleUpdate} />
      ...
      {/* Conversations */}
      <ConversationTimeline conversations={conversations} loading={conversationsLoading} />
    </div>
  );
}
