...
export async function listLeads(
  ...
  try {
    const supabase = await createClient()
    const { page, limit, status, search, orderBy, order } = params
...
export async function getLeadStats(
  ...
  try {
    ...
    // Count by status
    const byStatus = (leads || []).reduce((acc, lead: any) => {
    ...
    const averageScore = leadsWithScore.length > 0
      ? leadsWithScore.reduce((sum, lead: any) => sum + (lead.score || 0), 0) / leadsWithScore.length
      : null
