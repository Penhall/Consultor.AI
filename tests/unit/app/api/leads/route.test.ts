...
describe('GET /api/leads', () => {
  ...
  it('deve listar leads com paginação padrão', async () => {
    ...
    expect(leadService.listLeads).toHaveBeenCalledWith('consultant-test-1', expect.objectContaining({
      page: 1,
      limit: 20,
    }))
  })
  ...
  it('deve filtrar por status', async () => {
    ...
    // Act
    const request = new NextRequest('http://localhost:3000/api/leads?page=1&limit=20&status=qualificado')
    const response = await GET(request)
    const data = await response.json()
  ...
  it('deve ordenar por campo especificado', async () => {
    ...
    // Act
    const request = new NextRequest('http://localhost:3000/api/leads?page=1&limit=20&orderBy=score&order=asc')
    const response = await GET(request)
    const data = await response.json()
