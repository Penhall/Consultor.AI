...
import type { ChoiceStep, ExecuteStep, MessageStep, ConversationState } from '@/lib/flow-engine/types'
...
describe('Step Executors', () => {
  ...
  it('executeChoiceStep retorna pergunta e opções', () => {
    ...
    if (result.success && result.type === 'choice') {
  ...
  it('executeActionStep executa ação de IA e retorna action_complete', async () => {
    ...
    if (result.success && result.type === 'action_complete') {
