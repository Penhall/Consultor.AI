...
vi.mock('@google/generative-ai', () => {
  const mockGenerateContent = vi.fn().mockResolvedValue({
    response: {
      text: () =>
        'Olá! Entendo que você busca um plano de saúde. Para te ajudar melhor, vou preparar algumas opções adequadas ao seu perfil. Posso te enviar as opções?',
    },
  });
  ...
  describe('getFallbackResponse', () => {
    ...
    it('fallback NÃO deve conter preços exatos', () => {
      ...
      verticals.forEach(vertical => {
        ...
        const pricePatterns = [/R\$\s*\d+[.,]?\d*/gi, /\d+\s*reais/gi];
    ...
    it('fallback NÃO deve pedir dados sensíveis', () => {
      ...
      verticals.forEach(vertical => {
        ...
        const sensitivePatterns = [/cpf/gi, /rg/gi, /senha/gi, /cartão de crédito/gi];
